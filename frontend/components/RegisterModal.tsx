...
interface RegisterModalProps {
  guestMessages: ChatMessage[]; // <-- 1. Принимаем гостевые сообщения
  onRegisterSuccess: (user: User) => void;
  onClose: () => void;
}

const RegisterModal: React.FC<RegisterModalProps> = ({ guestMessages, onRegisterSuccess, onClose }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // --- 2. ФОРМИРУЕМ ПАРЫ "ВОПРОС-ОТВЕТ" ИЗ ГОСТЕВОЙ ИСТОРИИ ---
      const messagesToSave = [];
      for (let i = 0; i < guestMessages.length; i++) {
        // Находим сообщение пользователя и убеждаемся, что следующее сообщение - от бота
        if (
          guestMessages[i].role === 'user' &&
          guestMessages[i+1]?.role === 'bot' &&
          // Исключаем специальные сообщения-маркеры
          guestMessages[i+1]?.text !== 'invite_to_register'
        ) {
          messagesToSave.push({
              request_text: guestMessages[i].text,
              response_text: guestMessages[i+1].text
          });
        }
      }

      // --- 3. ОТПРАВЛЯЕМ ДАННЫЕ НА БЭКЕНД, ВКЛЮЧАЯ ИСТОРИЮ ---
      const userFromApi = await createUser({
        first_name: firstName,
        last_name: lastName || null,
        dob: birthDate,
        phone: phone,
        guest_messages: messagesToSave, // <-- Передаем историю
      });
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-md p-8 space-y-8 bg-slate-800 rounded-2xl shadow-2xl ring-1 ring-white/10 relative"
      ...
      >
        ...
        <div className="text-center">
          ...
          <h1 className="mt-4 text-3xl font-bold tracking-tight text-white sm-text-4xl">Личный кабинет</h1>
          <p className="mt-2 text-lg text-slate-400">Сохраните историю ваших снов</p>
        </div>
